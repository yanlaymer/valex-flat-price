...
class Predictor:
    ...
    def predict_price(self):
        ...
        logger.info(
            f"PRICE (WITHOUT CORRECTION): {prediction * self.entry['total_square']}"
        )
        logger.info(f"PRICE PER SQUARE METERS (WITHOUT CORRECTION): {prediction}")

        analog_min_price = (
            self.model_entry["analog_prices_min"] * self.entry["total_square"]
        )
        analog_median_price = (
            self.model_entry["analog_prices_median"] * self.entry["total_square"]
        )
        analog_max_price = (
            self.model_entry["analog_prices_max"] * self.entry["total_square"]
        )
        ...
        logger.info(
            f"PRICE PER SQUARE METERS (AFTER CORRECTION): {corrected_price / self.entry['total_square']}"
        )
        analog_links = [
            self.model_entry["analog_1"],
            self.model_entry["analog_2"],
